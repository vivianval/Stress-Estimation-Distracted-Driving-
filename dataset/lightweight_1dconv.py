...
# # For each subject s and phase p:
# #   - Load phased MD and phased ND
# #   - For each phase segment, smooth feature trajectories (spline or triangular conv)
# #   - Compute:
# #       mu  = mean_t Y(t)
# #       nu  = mean_t |dY/dt|     (matches your paragraph)
# #   - Form paired deltas:
# #       Delta_mu(s,p,f) = mu_MD - mu_ND
# #       Delta_nu(s,p,f) = nu_MD - nu_ND
# # Across subjects:
# #   - Paired t-test of deltas vs 0 for each (p,f,metric)
# #   - Save CSV with p-values + Bonferroni + flags
# # """
...
# # # Optional csaps smoothing (preferred)
# # try:
...
# # @dataclass
# # class Config:
# #     phased_dir: str = "dataset/paired_tests_EMOCA/phased_csvs"  # <- your PHASED_DIR
# #     subjects: Optional[List[str]] = None

# #     # Which features to use:
# #     # If None -> auto-detect exp_/pose_ only (optionally delta_pose_)
# #     include_delta_pose: bool = False

# #     # Phase column in your phased CSVs
# #     phase_col: str = "Phase"     # values like "P1"..."P5"
# #     time_hz: float = 10.0        # your EMOCA sampling rate (dt = 1/time_hz)

# #     # Smoothing mode
# #     mode: str = "conv"           # "conv" or "spline"
# #     conv_kernel: int = 5         # triangular kernel size
# #     spline_p: float = 0.8        # csaps smooth in [0,1]

# #     # Minimum samples per phase (avoid tiny fragments)
# #     min_len: int = 40
...
# #             cols.append(cs)
# #     # Keep stable order: pose then exp then delta_pose
...
# #     # fallback: UnivariateSpline with s ~ (1-p)*N*var
# #     xx = x[mask]
...
# # def triangular_smooth_1d(x: np.ndarray, k: int) -> np.ndarray:
# #     if k <= 1:
# #         return x.copy()
...
# # def smooth_matrix(X: np.ndarray, mode: str, dt: float, conv_kernel: int, spline_p: float) -> Tuple[np.ndarray, np.ndarray]:
# #     T, F = X.shape
# #     if T < 2:
...
# #         for f in range(F):
# #             Y[:, f] = triangular_smooth_1d(X[:, f].astype(float), conv_kernel)
...
# #             Y[:, f]  = smooth_series_spline(t, col, p=spline_p, deriv=0)
# #             dY[:, f] = smooth_series_spline(t, col, p=spline_p, deriv=1) / dt
# #         return Y, dY

# #     raise ValueError("mode must be 'conv' or 'spline'")
...
# #     dt = 1.0 / cfg.time_hz
# #     Y, dY = smooth_matrix(X, cfg.mode, dt, cfg.conv_kernel, cfg.spline_p)

# #     mu = np.nanmean(Y, axis=0)
# #     # IMPORTANT: match your paragraph: nu = mean_t sqrt(dotY^2) = mean_t |dotY|
...
# # def main():
# #     cfg = Config(
# #         phased_dir="dataset/paired_tests_EMOCA/phased_csvs",
# #         include_delta_pose=False,  # set True if you want delta_pose_ too
# #         mode="conv",               # conv is what you’re refining
# #         conv_kernel=5,
# #         spline_p=0.8,
# #         time_hz=10.0,
# #         min_len=40,
# #         out_csv="phasewise_md_nd_phased_smoothed_stats.csv",
# #     )

# #     phased_dir = Path(cfg.phased_dir)
# #     subs = cfg.subjects if cfg.subjects is not None else list_subjects(phased_dir)
# #     if not subs:
...
# #         else:
# #             # enforce same feature order across subjects
...
# #         raise SystemExit("No subjects processed successfully.")

# #     print(f"Loaded {len(all_subj)} subjects.")
...
def main():
    # === Run the three settings you care about ===
    settings = [
